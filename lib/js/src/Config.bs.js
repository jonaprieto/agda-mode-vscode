...
function setCommandLineOptions(options) {
  return Vscode.workspace.getConfiguration("agdaMode", undefined).update("connection.commandLineOptions", options, 1, undefined);
}
...
var Connection = {
  ...
  setCommandLineOptions: setCommandLineOptions,
...
function getTypecheckOnOpen() {
  var raw = Vscode.workspace.getConfiguration("agdaMode", undefined).get("typecheckOnOpen");
  if (raw !== undefined && !raw) {
    return false;
  } else {
    return true;
...
function getTypecheckOnSave() {
  var raw = Vscode.workspace.getConfiguration("agdaMode", undefined).get("typecheckOnSave");
  if (raw !== undefined && !raw) {
    return false;
  } else {
    return true;
...
function getShowErrorNotifications() {
  var raw = Vscode.workspace.getConfiguration("agdaMode", undefined).get("showErrorNotifications");
  if (raw !== undefined && !raw) {
    return false;
  } else {
    return true;
...
function getBackend$1() {
  var raw = Vscode.workspace.getConfiguration("agdaMode", undefined).get("compile.backend");
  if (raw === undefined) {
    return "GHC";
  }
  switch (raw) {
...
function setBackend(backend) {
  return Vscode.workspace.getConfiguration("agdaMode", undefined).update("compile.backend", Common$AgdaModeVscode.Backend.toString(backend), 1, undefined);
}

function getGhcOptions() {
  return Core__Option.getOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("compile.ghcOptions"), "");
}

function setGhcOptions(options) {
  return Vscode.workspace.getConfiguration("agdaMode", undefined).update("compile.ghcOptions", options, 1, undefined);
}

function getOutputPath() {
  return Core__Option.getOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("compile.outputPath"), "");
}

function setOutputPath(path) {
  return Vscode.workspace.getConfiguration("agdaMode", undefined).update("compile.outputPath", path, 1, undefined);
}

function getOptions() {
  return {
          backend: getBackend$1(),
          ghcOptions: getGhcOptions(),
          mainModule: "",
          runCommand: "./Main",
          outputPath: getOutputPath(),
          htmlOptions: Common$AgdaModeVscode.HtmlOptions.$$default,
          latexOptions: Common$AgdaModeVscode.LatexOptions.$$default
        };
}

async function setOptions(options) {
  await setBackend(options.backend);
  await setGhcOptions(options.ghcOptions);
  await setOutputPath(options.outputPath);
}

var Compile = {
  getBackend: getBackend$1,
  setBackend: setBackend,
  getGhcOptions: getGhcOptions,
  setGhcOptions: setGhcOptions,
...
exports.getTypecheckOnOpen = getTypecheckOnOpen;
exports.getTypecheckOnSave = getTypecheckOnSave;
exports.getShowErrorNotifications = getShowErrorNotifications;
