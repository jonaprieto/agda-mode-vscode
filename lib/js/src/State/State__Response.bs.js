...
var State__Diagnostics$AgdaModeVscode = require("./State__Diagnostics.bs.js");
...
async function handle(state, x) {
  switch (x.TAG) {
    ...
    case "CompilationOk" :
        var body = x._0;
        var outputLines = body.split("\n");
        await State__View$AgdaModeVscode.Panel.setCompileOutput(state, outputLines, false);
        await State__View$AgdaModeVscode.Panel.setActiveTab(state, "Compile");
        return await State__View$AgdaModeVscode.Panel.display(state, {
                    TAG: "Success",
                    _0: "Compiled ✓"
                  }, [Item$AgdaModeVscode.plainText(body)]);
    case "CompilationOkALS" :
        var errors = x._1;
        var warnings = x._0;
        var allOutput = ["The module was successfully compiled."].concat(warnings.concat(errors));
        var hasErrors = errors.length > 0;
        await State__View$AgdaModeVscode.Panel.setCompileOutput(state, allOutput, hasErrors);
        await State__View$AgdaModeVscode.Panel.setActiveTab(state, "Compile");
        var message = [Item$AgdaModeVscode.plainText("The module was successfully compiled.")];
        var errors$1 = errors.map(function (raw) {
              return Item$AgdaModeVscode.error(RichText$AgdaModeVscode.string(raw), raw);
            });
        var warnings$1 = warnings.map(function (raw) {
    ...
    case "Constraints" :
        ...
        var items = Emacs__Parser2$AgdaModeVscode.parseOutputs(body$1);
        return await State__View$AgdaModeVscode.Panel.display(state, {
                    TAG: "Plain",
                    _0: "Constraints"
                  }, items);
    case "AllGoalsWarnings" :
        ...
        var _header = x._0;
        if (body$2 === "nil") {
          State__Diagnostics$AgdaModeVscode.clearForUri(state.document.uri);
          return await State__View$AgdaModeVscode.Panel.display(state, {
                      TAG: "Success",
                      _0: "Typecheck successful ✓"
                    }, []);
        }
        var parsed = Emacs__Parser2$AgdaModeVscode.parseAllGoalsWarnings(_header, body$2);
        State__Diagnostics$AgdaModeVscode.clearForUri(state.document.uri);
        var errors$2 = Core__Option.getOr(parsed["errors"], []);
        var warnings$2 = Core__Option.getOr(parsed["warnings"], []);
        errors$2.forEach(function (raw) {
              State__Diagnostics$AgdaModeVscode.addError(state.document, raw, raw);
            });
        warnings$2.forEach(function (raw) {
              State__Diagnostics$AgdaModeVscode.addWarning(state.document, raw, raw);
            });
        if (errors$2.length > 0 && Config$AgdaModeVscode.getShowErrorNotifications()) {
          var fileName = Nodepath.basename(state.document.fileName);
          var errorCount = errors$2.length;
          var message$1 = errorCount === 1 ? "Agda: 1 error in " + fileName : "Agda: " + errorCount.toString() + " errors in " + fileName;
          Vscode.window.showErrorMessage(message$1);
        }
        var items$1 = Emacs__Parser2$AgdaModeVscode.render(parsed);
        return await State__View$AgdaModeVscode.Panel.display(state, {
                    TAG: "Plain",
                    _0: _header
                  }, items$1);
    case "AllGoalsWarningsALS" :
        ...
        var warnings$3 = x._3;
        State__Diagnostics$AgdaModeVscode.clearForUri(state.document.uri);
        errors$3.forEach(function (raw) {
              State__Diagnostics$AgdaModeVscode.addError(state.document, raw, raw);
            });
        warnings$3.forEach(function (raw) {
              State__Diagnostics$AgdaModeVscode.addWarning(state.document, raw, raw);
            });
        if (errors$3.length > 0 && Config$AgdaModeVscode.getShowErrorNotifications()) {
          var fileName$1 = Nodepath.basename(state.document.fileName);
          var errorCount$1 = errors$3.length;
          var message$2 = errorCount$1 === 1 ? "Agda: 1 error in " + fileName$1 : "Agda: " + errorCount$1.toString() + " errors in " + fileName$1;
          Vscode.window.showErrorMessage(message$2);
        }
        var errors$4 = errors$3.map(function (raw) {
              return Item$AgdaModeVscode.error(RichText$AgdaModeVscode.string(raw), raw);
            });
        var warnings$4 = warnings$3.map(function (raw) {
              return Item$AgdaModeVscode.warning(RichText$AgdaModeVscode.string(raw), raw);
            });
        return await State__View$AgdaModeVscode.Panel.display(state, {
                    TAG: "Plain",
    ...
    case "Error" :
        var body$3 = x._0;
        State__Diagnostics$AgdaModeVscode.clearForUri(state.document.uri);
        State__Diagnostics$AgdaModeVscode.addError(state.document, body$3, body$3);
        if (Config$AgdaModeVscode.getShowErrorNotifications()) {
          var fileName$2 = Nodepath.basename(state.document.fileName);
          var truncatedError = body$3.length > 500 ? body$3.slice(0, 500) + "..." : body$3;
          var messageOptions_modal = false;
          var messageOptions_detail = truncatedError;
          var messageOptions = {
            modal: messageOptions_modal,
            detail: messageOptions_detail
          };
          Vscode.window.showErrorMessage("Agda: Type error in " + fileName$2, messageOptions);
        }
        var items$3 = Emacs__Parser2$AgdaModeVscode.render(Emacs__Parser2$AgdaModeVscode.parseError(body$3));
        return await State__View$AgdaModeVscode.Panel.display(state, {
                    TAG: "Error",
                    _0: "Error"
                  }, items$3);
...
async function handle$1(state, dispatchCommand, response) {
  ...
  State__StatusBar$AgdaModeVscode.handleResponse(response);
