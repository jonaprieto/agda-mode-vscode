...
function activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri, memento) {
  ...
  subscribe(onOpenEditor(function (editor) {
            ...
            if (!Config$AgdaModeVscode.getTypecheckOnOpen()) {
              return ;
            }
            ...
            State__Command$AgdaModeVscode.dispatchCommand(state$1, "Load");
          }));
  ...
  subscribe(Vscode.workspace.onDidSaveTextDocument(function ($$document) {
            if (isAgda($$document) && Config$AgdaModeVscode.getTypecheckOnSave()) {
              return Core__Option.forEach(Core__Option.flatMap(Registry$AgdaModeVscode.get($$document), (function (entry) {
                                return entry.state;
                              })), (function (state) {
                            State__Command$AgdaModeVscode.dispatchCommand(state, "Load");
                          }));
            }
